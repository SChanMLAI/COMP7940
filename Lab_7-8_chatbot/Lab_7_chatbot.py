...
import logging
import redis
from Lab_7_ChatGPT_HKBU import HKBU_ChatGPT
...
def main():
    ...
    global redis1
    # redis1 = redis.Redis(host=(config['REDIS']['HOST']), password=(config['REDIS']['PASSWORD']), port=int(config['REDIS']['REDISPORT']))
    redis1 = redis.Redis(host=(os.environ['REDIS_HOST']), password=(os.environ['REDIS_PASSWORD']), port=int(os.environ['REDIS_REDISPORT']))
